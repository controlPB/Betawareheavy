...
local Window = Rayfield:CreateWindow({
    Name = "Betaware Heavy Hub | Arsenal",
    LoadingTitle = "Betaware Heavy",
    LoadingSubtitle = "by control#4788",
    ConfigurationSaving = {
       Enabled = true,
    ...
    KeySettings = {
       ...
       Key = {"Betaware,heavy"} -- List of keys that will be accepted by the system, can be RAW file links (pastebin, github etc) or simple strings ("hello","key22")
 ...
 local Tab = Window:CreateTab("Aimbot", 13841375016) -- Title, Image
 local Toggle = Tab:CreateToggle({
   Name = "Betaware Menu",
   CurrentValue = false,
   Flag = "Toggle1", -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps
   Callback = function(Value)-- Issues:
    ...
    function SaveDarkHubV1Settings()
        local temp = { }
        local succ, out = pcall(function()
            temp.TracersLength = DarkHubV1.TracersLength
            temp.ESPLength = DarkHubV1.ESPLength
            temp.CHAMSLength = DarkHubV1.CHAMSLength
            temp.PlaceTracersUnderCharacter = DarkHubV1.PlaceTracersUnderCharacter
            temp.FreeForAll = DarkHubV1.FreeForAll
            temp.AutoFire = DarkHubV1.AutoFire
            temp.AimbotKey = tostring(DarkHubV1.AimbotKey)
            temp.MobChams = DarkHubV1.MobChams
            temp.MobESP = DarkHubV1.MobESP
            temp.Colors = { }
            for i, v in next, DarkHubV1.Colors do
                temp.Colors[i] = tostring(v)
            end
            writefile("ProjectDarkHubV1.txt", HTTP:JSONEncode(temp))
        end)
        if not succ then
            error(out)
        end
    ...
    fuck = pcall(function()
        local temp = HTTP:JSONDecode(readfile("ProjectDarkHubV1.txt"))
        if temp.MobChams ~= nil and temp.MobESP ~= nil then
            ...
            DarkHubV1.Colors = temp.Colors
        else
            spawn(function()
                SaveDarkHubV1Settings()
        ...
        DarkHubV1.AutoFire = false
    end)
    
    -- Load blacklist file if it exists
    fuck2 = pcall(function()
        DarkHubV1.Blacklist = HTTP:JSONDecode(readfile("Blacklist.txt"))
    end)
    
    fuck3 = pcall(function()
        DarkHubV1.FriendList = HTTP:JSONDecode(readfile("Whitelist.txt"))
    end)
    ...
    if #Teams:GetChildren() <= 0 then
        DarkHubV1.FreeForAll = true
    end
    
    if DarkHubV1.TracersLength > 2048 then
        DarkHubV1.TracersLength = 2048
    end
    
    if DarkHubV1.CHAMSLength > 2048 then
        DarkHubV1.CHAMSLength = 2048
    end
    ...
    function GetTeamColor(Plr)
        ...
        local PickedColor = DarkHubV1.Colors.Enemy
        
        if Plr ~= nil then
            if game.PlaceId == 606849621 then
                if DarkHubV1.Colors.ColorOverride == nil then
                    if not DarkHubV1.FreeForAll then
                        if MyPlr.Team ~= nil and Plr.Team ~= nil then
                            if DarkHubV1.FriendList[Plr.Name] == nil then
                                if MyPlr.Team.Name == "Prisoner" then
                                    if Plr.Team == MyPlr.Team or Plr.Team.Name == "Criminal" then
                                        PickedColor = DarkHubV1.Colors.Ally
                                    else
                                        PickedColor = DarkHubV1.Colors.Enemy
                                    end
                                ...
                                elseif MyPlr.Team.Name == "Police" then
                                    if Plr.Team == MyPlr.Team then
                                        ...
                                        elseif Plr.Team.Name == "Prisoner" then
                                            PickedColor = DarkHubV1.Colors.Neutral
                                        end
                                    end
                                end
                            else
                                PickedColor = DarkHubV1.Colors.Friend
                            end
                        end
                    else
                        if DarkHubV1.FriendList[Plr.Name] ~= nil then
                    ...
                    PickedColor = DarkHubV1.Colors.ColorOverride
                end
                        ...
                        if DarkHubV1.FriendList[Plr.Name] == nil then
                            ...
                            elseif MyPlr.Team.Name == "Guards" then
                                ...
                                elseif Plr.Team.Name == "Guards" then
                                    PickColor = DarkHubV1.Colors.Ally
                                end
            ...
            elseif game.PlaceId == 379614936 then
                if DarkHubV1.Colors.ColorOverride == nil then
                    if not DarkHubV1.FriendList[Plr.Name] then
            ...
            elseif game.PlaceId == 983224898 then
                if (tick() - wildrevolvertick) > 10 or wildrevolverteamdata == nil then
                    ...
                    return DarkHubV1.Colors.Neutral
                end
                ...
                if DarkHubV1.Colors.ColorOverride == nil then
                    if not DarkHubV1.FreeForAll then
                        if MyPlr.Team ~= Plr.Team and not DarkHubV1.FriendList[Plr.Name] then
                            PickedColor = DarkHubV1.Colors.Enemy
                        elseif MyPlr.Team == Plr.Team and not DarkHubV1.FriendList[Plr.Name] then
                            PickedColor = DarkHubV1.Colors.Ally
                        else
                            PickedColor = DarkHubV1.Colors.Friend
                        end
    ...
    function GetClosestPlayerNotBehindWall()
        ...
        for _, v in next, Plrs:GetPlayers() do
            if v ~= MyPlr and not DarkHubV1.Blacklist[v.Name] then
                local IsAlly = GetTeamColor(v)
                if IsAlly ~= DarkHubV1.Colors.Ally and IsAlly ~= DarkHubV1.Colors.Friend and IsAlly ~= DarkHubV1.Colors.Neutral then
            ...
            if Find then
                ...
                if MyChar and GetChar then
                    ...
                    if Find2 and Find3 then
                        ...
                        if Dist > DarkHubV1.ESPLength or DarkHubV1.Blacklist[Plr.Name] then
    ...
    function CreateESP(Obj)
        if Obj ~= nil then
            ...
            local PickColor = GetTeamColor(Obj) or DarkHubV1.Colors.Neutral
    ...
    function UpdateTracer(Plr)
        if DarkHubV1.TracersEnabled then
            if MyChar then
                local MyTor = MyChar:FindFirstChild("HumanoidRootPart")
                local GetTor = TracerData[Plr.Name]
                if MyTor and GetTor ~= nil and GetTor.Parent ~= nil then
                    local Dist = (MyTor.Position - GetTor.Position).magnitude
                    if (Dist < DarkHubV1.TracersLength and not DarkHubV1.Blacklist[Plr.Name]) and not (MyChar:FindFirstChild("InVehicle") or GetTor.Parent:FindFirstChild("InVehicle")) then
                        if not DarkHubV1.PlaceTracersUnderCharacter then
    ...
    function CreateTracers(Plr)
        ...
        if not Find then
            ...
            B.ZIndex = 5
            B.Transparency = 0
            B.Color3 = GetTeamColor(Plr) or DarkHubV1.Colors.Neutral
            P.Parent = Tracers
        ...
        if Obj:IsA("Player") then
            ...
            if GetChar and MyChar then
                ...
                if GetHead and GetTor and MyHead and MyTor then
                    if (MyTor.Position - GetTor.Position).magnitude > DarkHubV1.CHAMSLength or DarkHubV1.Blacklist[Obj.Name] then
                        Trans = 1
                    else
            ...
            if Find then
                for i, v in next, Find:GetChildren() do
                    if v.className ~= "ObjectValue" then
                        v.Color3 = GetTeamColor(Obj) or DarkHubV1.Colors.Neutral
        ...
        if Obj ~= nil then
            ...
            for _, P in next, Crap do
                if P:IsA("PVInstance") and P.Name ~= "HumanoidRootPart" then
                    ...
                    Box.ZIndex = 5
                    Box.Transparency = 0
                    Box.Color3 = ((IsPlr) and GetTeamColor(Plrs:GetPlayerFromCharacter(Obj))) or DarkHubV1.Colors.Neutral
                    Box.Parent = FindC
                end
            end
        end
    ...
    function CreateMobESPChams()
        ...
        for i, v in next, workspace:GetDescendants() do
            ...
            if hum and not Plrs:GetPlayerFromCharacter(hum.Parent) and FindCham(v) == nil and FindESP(v) == nil then
                ...
                if DarkHubV1.CHAMSEnabled and DarkHubV1.MobChams then
                    CreateChams(v)
                end
                if DarkHubV1.ESPEnabled and DarkHubV1.MobESP then
                    CreateESP(v)
                end
            end
        end
    
        if DarkHubV1.Mob_ESP_CHAMS_Ran_Once == false then
            for i, v in next, mobspawn do
                v.ChildAdded:connect(function(Obj)
                    if DarkHubV1.MobChams then
                        local t = tick()
        ...
        Obj.ChildAdded:connect(function(Obj2)
            if DarkHubV1.OutlinesEnabled then
                if Obj2:IsA("BasePart") and not Plrs:GetPlayerFromCharacter(Obj2.Parent) and not Obj2.Parent:IsA("Hat") and not Obj2.Parent:IsA("Accessory") and Obj2.Parent.Name ~= "Tracers" then
                    ...
                    outline.Parent = Obj2
                    Data[1] = outline
                    rawset(DarkHubV1.OutlinedParts, Obj2, Data)
                end
                ...
                for i, v in next, Obj2:GetDescendants() do
                    if v:IsA("BasePart") and not Plrs:GetPlayerFromCharacter(v.Parent) and not v.Parent:IsA("Hat") and not v.Parent:IsA("Accessory") and v.Parent.Name ~= "Tracers" then
                        ...
                        outline.Parent = v
                        Data[1] = outline
                        rawset(DarkHubV1.OutlinedParts, v, Data)
                    end
    ...
    function LightingHax()
        ...
        if DarkHubV1.FullbrightEnabled then
    ...
    Plrs.PlayerAdded:connect(function(Plr)
        if DarkHubV1.CharAddedEvent[Plr.Name] == nil then
            DarkHubV1.CharAddedEvent[Plr.Name] = Plr.CharacterAdded:connect(function(Char)
                if DarkHubV1.ESPEnabled then
                    RemoveESP(Plr)
                    CreateESP(Plr)
                end
                if DarkHubV1.CHAMSEnabled then
                    RemoveChams(Plr)
                    CreateChams(Plr)
                end
                if DarkHubV1.TracersEnabled then
    ...
    Plrs.PlayerRemoving:connect(function(Plr)
        if DarkHubV1.CharAddedEvent[Plr.Name] ~= nil then
            DarkHubV1.CharAddedEvent[Plr.Name]:Disconnect()
            DarkHubV1.CharAddedEvent[Plr.Name] = nil
        end
        RemoveESP(Plr)
        RemoveChams(Plr)
        RemoveTracers(Plr)
    ...
    function InitMain()
        ...
        local DarkHubV120 = Instance.new("ScreenGui")
        ...
        DarkHubV120.Name = "DarkHubV1 3.0"
        DarkHubV120.Parent = CoreGui
        DarkHubV120.ResetOnSpawn = false
        
        MainFrame.Name = "MainFrame"
        MainFrame.Parent = DarkHubV120
        ...
        Title.Text = "Betaware Heavy | By control#4788"
    ...
    11/10/2022: Script Published
        ...
        AllyColor.Text = tostring(DarkHubV1.Colors.Ally)
        ...
        CHAMSLength.Text = tostring(DarkHubV1.CHAMSLength)
        ...
        CrosshairColor.Text = tostring(DarkHubV1.Colors.Crosshair)
        ...
        ESPLength.Text = tostring(DarkHubV1.ESPLength)
        ...
        EnemyColor.Text = tostring(DarkHubV1.Colors.Enemy)
        ...
        FreeForAll.Text = tostring(DarkHubV1.FreeForAll)
        ...
        FriendColor.Text = tostring(DarkHubV1.Colors.Friend)
        ...
        NeutralColor.Text = tostring(DarkHubV1.Colors.Neutral)
        ...
        TracersLength.Text = tostring(DarkHubV1.TracersLength)
        ...
        TracersUnderChars.Text = tostring(DarkHubV1.PlaceTracersUnderCharacter)
        ...
        AutoFireToggle.Text = tostring(DarkHubV1.AutoFire)
        ...
        AimbotKey.Text = tostring(DarkHubV1.AimbotKey)
        ...
        MobESPButton.Text = tostring(DarkHubV1.MobESP)
        ...
        MobChamsButton.Text = tostring(DarkHubV1.MobChams)
        ...
        RefreshPlayerLabels(players, DarkHubV1.Blacklist)
        RefreshPlayerLabels(players2, DarkHubV1.FriendList)
        
        ESPToggle.MouseButton1Click:connect(function()
            DarkHubV1.ESPEnabled = not DarkHubV1.ESPEnabled
            if DarkHubV1.ESPEnabled then
                ESPToggle.BackgroundColor3 = Color3.new(0/255,171/255,11/255)
                for _, v in next, Plrs:GetPlayers() do
                    if v ~= MyPlr then
                        if DarkHubV1.CharAddedEvent[v.Name] == nil then
                            DarkHubV1.CharAddedEvent[v.Name] = v.CharacterAdded:connect(function(Char)
                                if DarkHubV1.ESPEnabled then
                                    RemoveESP(v)
                                    CreateESP(v)
                                end
        ...
        ChamsToggle.MouseButton1Click:connect(function()
            DarkHubV1.CHAMSEnabled = not DarkHubV1.CHAMSEnabled
            if DarkHubV1.CHAMSEnabled then
        ...
        TracersToggle.MouseButton1Click:connect(function()
            DarkHubV1.TracersEnabled = not DarkHubV1.TracersEnabled
            if DarkHubV1.TracersEnabled then
        ...
        DebugToggle.MouseButton1Click:connect(function()
            DarkHubV1.DebugInfo = not DarkHubV1.DebugInfo
            DebugMenu["Main"].Visible = DarkHubV1.DebugInfo
            if DarkHubV1.DebugInfo then
        ...
        OutlineToggle.MouseButton1Click:connect(function()
            DarkHubV1.OutlinesEnabled = not DarkHubV1.OutlinesEnabled
            if DarkHubV1.OutlinesEnabled then
                ...
                if DarkHubV1.LightingEvent == nil then
                    DarkHubV1.LightingEvent = game:GetService("Lighting").Changed:connect(LightingHax)
                end
            else
                OutlineToggle.BackgroundColor3 = Color3.new(1, 1, 1)
                for i, v in next, DarkHubV1.OutlinedParts do
                    i.Transparency = v[2]
                    v[1]:Destroy()
                end
            end
        ...
        FullbrightToggle.MouseButton1Click:connect(function()
            DarkHubV1.FullbrightEnabled = not DarkHubV1.FullbrightEnabled
            if DarkHubV1.FullbrightEnabled then
                FullbrightToggle.BackgroundColor3 = Color3.new(0/255,171/255,11/255)
                if DarkHubV1.LightingEvent == nil then
                    DarkHubV1.LightingEvent = Light.Changed:connect(LightingHax)
                end
            else
                FullbrightToggle.BackgroundColor3 = Color3.new(1, 1, 1)
                Light.Ambient = DarkHubV1.AmbientBackup
                Light.ColorShift_Bottom = DarkHubV1.ColorShiftBotBackup
                Light.ColorShift_Top = DarkHubV1.ColorShiftTopBackup
            end
        end)
    
        Crosshair.MouseButton1Click:connect(function()
            DarkHubV1.CrosshairEnabled = not DarkHubV1.CrosshairEnabled
            if DarkHubV1.CrosshairEnabled then
                local g = Instance.new("ScreenGui", CoreGui)
                g.Name = "Corsshair"
                local line1 = Instance.new("TextLabel", g)
                line1.Text = ""
                line1.Size = UDim2.new(0, 35, 0, 1)
                line1.BackgroundColor3 = DarkHubV1.Colors.Crosshair
                line1.BorderSizePixel = 0
                line1.ZIndex = 10
                local line2 = Instance.new("TextLabel", g)
                line2.Text = ""
                line2.Size = UDim2.new(0, 1, 0, 35)
                line2.BackgroundColor3 = DarkHubV1.Colors.Crosshair
        ...
        AimbotToggle.MouseButton1Click:connect(function()
            if not (game.PlaceId == 292439477 or game.PlaceId == 606849621) then
                DarkHubV1.AimbotEnabled = not DarkHubV1.AimbotEnabled
                if DarkHubV1.AimbotEnabled then
        ...
        TracersUnderChars.MouseButton1Click:connect(function()
            DarkHubV1.PlaceTracersUnderCharacter = not DarkHubV1.PlaceTracersUnderCharacter
            if DarkHubV1.PlaceTracersUnderCharacter then
        ...
        FreeForAll.MouseButton1Click:connect(function()
            DarkHubV1.FreeForAll = not DarkHubV1.FreeForAll
            if DarkHubV1.FreeForAll then
        ...
        EnemyColor.FocusLost:connect(function()
            ...
            if R ~= nil and G ~= nil and B ~= nil then
                if not (R > 1 and G > 1 and B > 1) and not (R < 0 and G < 0 and B < 0) then
                    DarkHubV1.Colors.Enemy = Color3.new(R, G, B)
                    EnemyColor.Text = tostring(DarkHubV1.Colors.Enemy)
                else
                    EnemyColor.Text = tostring(DarkHubV1.Colors.Enemy)
                end
        ...
        AllyColor.FocusLost:connect(function()
            ...
            if R ~= nil and G ~= nil and B ~= nil then
                if not (R > 1 and G > 1 and B > 1) and not (R < 0 and G < 0 and B < 0) then
                    DarkHubV1.Colors.Ally = Color3.new(R, G, B)
                    AllyColor.Text = tostring(DarkHubV1.Colors.Ally)
                else
                    AllyColor.Text = tostring(DarkHubV1.Colors.Ally)
                end
        ...
        CrosshairColor.FocusLost:connect(function()
            ...
            if R ~= nil and G ~= nil and B ~= nil then
                if not (R > 1 and G > 1 and B > 1) and not (R < 0 and G < 0 and B < 0) then
                    DarkHubV1.Colors.Ally = Color3.new(R, G, B)
                    EnemyColor.Text = tostring(DarkHubV1.Colors.Crosshair)
                else
                    EnemyColor.Text = tostring(DarkHubV1.Colors.Crosshair)
                end
            else
        ...
        AutoFireToggle.MouseButton1Click:connect(function()
            ...
            wait(3)
            hint:Destroy()
            --DarkHubV1.AutoFire = not DarkHubV1.AutoFire
            --AutoFireToggle.Text = tostring(DarkHubV1.AutoFire)
        end)
        ...
        AimbotKey.MouseButton1Click:connect(function()
            ...
            if input.UserInputType == Enum.UserInputType.Keyboard then
                DarkHubV1.AimbotKey = tostring(input.KeyCode)
                AimbotKey.Text = string.sub(tostring(input.KeyCode), 14)
            else
                DarkHubV1.AimbotKey = tostring(input.UserInputType)
                AimbotKey.Text = string.sub(tostring(input.UserInputType), 20)
            end
        end)
    
        MobESPButton.MouseButton1Click:connect(function()
            DarkHubV1.MobESP = not DarkHubV1.MobESP
            MobESPButton.Text = tostring(DarkHubV1.MobESP)
            if DarkHubV1.MobESP then
                local hint = Instance.new("Hint", CoreGui)
                hint.Text = "Turn ESP/Chams off and on again to see mob ESP."
                wait(5)
                hint.Text = "This is still in beta, expect problems! Message DΔRKぷ#0960 on discord if you encounter a bug!"
                wait(10)
                hint:Destroy()
            end
        end)
        ...
        MobChamsButton.MouseButton1Click:connect(function()
            DarkHubV1.MobChams = not DarkHubV1.MobChams
            MobChamsButton.Text = tostring(DarkHubV1.MobChams)
            if DarkHubV1.MobChams then
                local hint = Instance.new("Hint", CoreGui)
        ...
        Playername.FocusLost:connect(function()
            ...
            elseif not DarkHubV1.Blacklist[Playername.Text] then
        ...
        AddToBlacklist.MouseButton1Click:connect(function()
            local FindPlr = FindPlayer(Playername.Text)
            if FindPlr then
                if not DarkHubV1.Blacklist[FindPlr.Name] then
                    DarkHubV1.Blacklist[FindPlr.Name] = true
                    UpdateChams(FindPlr)
        ...
        RemoveToBlacklist.MouseButton1Click:connect(function()
            local FindPlr = FindPlayer(Playername.Text)
            if FindPlr then
                if DarkHubV1.Blacklist[FindPlr.Name] then
                    DarkHubV1.Blacklist[FindPlr.Name] = nil
                    UpdateChams(FindPlr)
                    RefreshPlayerLabels(players, DarkHubV1.Blacklist)
                end
            else
                if DarkHubV1.Blacklist[Playername.Text] then
                    DarkHubV1.Blacklist[Playername.Text] = nil
                    RefreshPlayerLabels(players, DarkHubV1.Blacklist)
                end
            end
        end)
        ...
        Playername2.FocusLost:connect(function()
            ...
            elseif not DarkHubV1.FriendList[Playername2.Text] then
        ...
        AddToWhitelist.MouseButton1Click:connect(function()
            local FindPlr = FindPlayer(Playername2.Text)
            if FindPlr then
                if not DarkHubV1.FriendList[FindPlr.Name] then
                    DarkHubV1.FriendList[FindPlr.Name] = true
                    UpdateChams(FindPlr)
        ...
        RemoveToWhitelist.MouseButton1Click:connect(function()
            local FindPlr = FindPlayer(Playername2.Text)
            if FindPlr then
                if DarkHubV1.FriendList[FindPlr.Name] then
                    DarkHubV1.FriendList[FindPlr.Name] = nil
                    UpdateChams(FindPlr)
                    RefreshPlayerLabels(players2, DarkHubV1.FriendList)
                end
            else
                if DarkHubV1.FriendList[Playername2.Text] then
                    DarkHubV1.FriendList[Playername2.Text] = nil
                    RefreshPlayerLabels(players2, DarkHubV1.FriendList)
                end
            end
        end)
        ...
        SaveWhitelist.MouseButton1Click:connect(function()
            pcall(function()
                writefile("Whitelist.txt", HTTP:JSONEncode(DarkHubV1.FriendList))
            end)
        ...
        SaveBlacklist.MouseButton1Click:connect(function()
            pcall(function()
                writefile("Blacklist.txt", HTTP:JSONEncode(DarkHubV1.Blacklist))
            end)
        ...
        UserInput.InputBegan:connect(function(input, ingui)
            if not ingui then
                ...
                if tostring(input.KeyCode) == DarkHubV1.AimbotKey or tostring(input.UserInputType) == DarkHubV1.AimbotKey then
                    DarkHubV1.Aimbot = true
                end
            end
        end)
    ...
    Run:BindToRenderStep("UpdateInfo", 1000, function()
        DarkHubV1.ClosestEnemy = GetClosestPlayer()
        ...
        if DarkHubV1.DebugInfo then
            ...
            if DarkHubV1.ClosestEnemy ~= nil then
                GetChar = DarkHubV1.ClosestEnemy.Character
                GetHead = GetChar:FindFirstChild("Head")
                GetTor = GetChar:FindFirstChild("HumanoidRootPart")
                GetHum = GetChar:FindFirstChild("Humanoid")
    
                DebugMenu["PlayerSelected"].Text = "Closest Enemy: " .. tostring(DarkHubV1.ClosestEnemy)
    
                if DarkHubV1.ClosestEnemy.Team ~= nil then
                    DebugMenu["PlayerTeam"].Text = "Team: " .. tostring(DarkHubV1.ClosestEnemy.Team)
                else
            ...
            local sum = 0
            local ave = 0
            table.insert(DarkHubV1.FPSAverage, fps)
            for i = 1, #DarkHubV1.FPSAverage do
                sum = sum + DarkHubV1.FPSAverage[i]
            end
            DebugMenu["FPS"].Text = "FPS: " .. tostring(fps) .. " Average: " .. string.format("%.0f", (sum / #DarkHubV1.FPSAverage))
            if (tick() - LastTick) >= 15 then
                DarkHubV1.FPSAverage = { }
                LastTick = tick()
            end
            LastTick = tick()
        end
    ...
    Run:BindToRenderStep("Aimbot", Enum.RenderPriority.First.Value, function()
        ...
        if DarkHubV1.AimbotEnabled and DarkHubV1.Aimbot then
            ...
            MyPlr.CameraMode = DarkHubV1.CameraModeBackup
        end
    end)
 ...
 Rayfield:Notify({
   Title = "Betaware",
   Content = "Made By control#4788 Thank you for using betaware.",
   Duration = 6.5,
   Image = 13928744176,
   Actions = { -- Notification Buttons
      Ignore = {
         Name = "Okay!",
         Callback = function()
         print("The user tapped Okay! Betaware!")
      end
   },
},
})
